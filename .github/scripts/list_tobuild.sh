      ...
      if [ ! -s tobuild.txt ] && [ -f "packages.json" ]; then
            ...
            if [ $counter -gt 4 ]; then
                ...
                if [ -s /tmp/resetpkgs ]; then
                    ...
                    cat /tmp/resetpkgs | xargs -i bash -c "pkgcounter=0; if [ -f \"logs/$(cat runstarttime)/retries/{}\" ]; then pkgcounter=$(<logs/$(cat runstarttime)/retries/{}); fi; pkgcounter=$((pkgcounter+1)); echo $pkgcounter > \"logs/$(cat runstarttime)/retries/{}\"; if [ $pkgcounter -lt 3 ]; then rm {}; fi"
                    git add lists
                    git add logs
                fi
            fi
